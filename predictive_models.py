...
class FTTransformer(nn.Module):
    ...
    def make_default(
        ...
        *,
        ...
        if activation_function is not 'default':
            for arg_name in [
                'ffn_activation',
                'head_activation',
            ]:
                transformer_config[arg_name] = locals()['activation_function']
        
        print(transformer_config['ffn_activation']) 
...
def apply_model(model, x_num, x_cat=None, return_features=False):
    ...
    """
    Apply a given model to input data.

    Parameters:
    - model (object): Prediction model. Supported types include FTTransformer, MLP, and ResNet.
    - x_num (tensor): Numerical input features.
    - x_cat (tensor, optional): Categorical input features. Default is None.
    - return_features (bool, optional): Whether to return additional features computed by the model. Default is False.

    Returns:
    - out_linear (tensor): Output from the linear layer of the model.
    - out_features (tensor or None): Additional features computed by the model if return_features is True, else None.

    Raises:
    - NotImplementedError: If the model type is not supported or custom, prompting implementation of the missing branch.
    """
...
def get_params(architecture_name, d_out, lr, weight_decay, num_features, activation_function):

    """
    Create a model and optimizer based on the specified architecture and parameters.

    Parameters:
    - architecture_name (str): Name of the architecture ('MLP', 'ResNet', or 'FTTransformer').
    - d_out (int): Dimensionality of the output.
    - lr (float): Learning rate for the optimizer.
    - weight_decay (float): Weight decay (L2 penalty) for the optimizer.
    - num_features (int): Dimensionality of the input features.
    - activation_function (function): Activation function to be used in the model.

    Returns:
    - model (object): Created machine learning model object.
    - optimizer (torch.optim.Optimizer): Optimizer object for training the model.

    Raises:
    - Exception: If the specified architecture_name is not supported.
    """
