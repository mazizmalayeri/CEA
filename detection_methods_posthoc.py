...
from predictive_models import apply_model
...
class ReactPostprocessor():
    def __init__(self, device, react_prob):
        self.percentile = react_prob
        self.device = device
        self.setup_flag = False
...
class CEA():
    def __init__(self, device, processor, percentile_top, addition_coef, treshold_caution_coef=1.1):
    
        """
        Args:
            device (str): Device for computation.
            processor (callable): Function for caluclating original novelty score, e.g., MSP.
            percentile_top (float): p parameter in CEA used for caluclating τ.
            addition_coef (float): γ parameter in CEA used for caluclating λ.
            treshold_caution_coef (float, optional): ρ parameter in CEA used for caluclating λ.
        """
    ...
    def setup(self, net, x_val, batch_size):
        """Calcaulating hyperparametrs based on a validation set from ID.
        ...
        if not self.setup_flag:
            ...
            with torch.no_grad():
                for batch_idx in range(len(x_val) // batch_size):
                    ...
                    added_score,original_score  = self.postprocess(net, data, val=True)
    ...
    @torch.no_grad()
    def postprocess(self, net: nn.Module, data: Any, val=False):
        ...
        Args:
            net (torch.nn.Module): Neural network model.
            data (Any): Input data.
            val (bool, optional): if true, the function reurns the original novelty score and added value of CEA seperately (default is false).

        Returns:
            Tuple: Tuple containing processed novelty score.
        """
        
        #Compute original novelty score
        pred, conf = self.processor(net, data)
        
        #Compute CEA added value
        _, feature = apply_model(net, data, return_features=True)
        nov = feature-self.threshold_top
        nov = nov.clip(min=0)
        nov = -torch.norm(nov, dim=1)
        
        if val:
            return nov.cpu().numpy(), conf
        else:
    ...
    def set_threshold(self):
        ...
        self.threshold_top = self.treshold_caution_coef*np.percentile(self.activation_log.flatten(),self.percentile_top)
        print('Top threshold at percentile {} over id data is: {}'.format(
            self.percentile_top, self.threshold_top))
...
def detection_method_wraper(detector, model, device, **arguments):
    ...
    **arguments: dict
    ...
    if detector == 'MSP':
        ...
        processor = ReactPostprocessor(device=device, react_prob=arguments['react_prob'])
...
def detection_method(detector, model, device, percentile_top=99.9, addition_coef=10, **arguments):

    """Adding CEA to a detection method.

    Args:
        detector: Original OOD detection method, e.g., MSP.
        model: Prediction model.
        device (str): Device for computation.
        percentile_top (float, optional): Percentile for capturing extreme activations (default is 99.9).
        addition_coef (float, optional): Coefficient for adjusting added CEA (default is 10).
        **arguments: Additional keyword arguments.

    Returns:
        Callable: Processor for detection method.
            If 'original' flag is True, returns the original processor directly.
            If 'original' flag is False, adds CEA to the processor.
    """
    
    processor = detection_method_wraper(detector, model, device, **arguments)
    
    if arguments['original']:
        return processor
    else:
        myprocessor = CEA(device=device, processor=processor, percentile_top=percentile_top, addition_coef=addition_coef)
        myprocessor.setup(model, arguments['x_val'], arguments['batch_size'])
        
        return myprocessor.postprocess